...
We then create a wrapper function to automate the process, so we can now simply add a CSV file name/path, and optionally a specific value we would like if enrollment is no longer the measure of interest, to the function my_wrapper(). Checking this output data to the data above, we can see our wrapper has reproduced the same data processing steps.
```{r}
